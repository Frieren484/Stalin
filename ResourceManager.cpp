...
bool ResourceManager::AddFile(const string& name, long long size, int catId, int ownerId) {
    // Group B: Path/Name Validation (std::regex)
    regex sysSymbols("[\\\\/:*?\"<>|]");
    ...
    if (!validExt) {
        cerr << "[ERROR] Extension is forbidden! Use .exe, .txt, .pdf" << endl;
        return false;
    }

    // Group B: Duplicate Check
    if (IsDuplicate(name)) {
...
void ResourceManager::CleanupOldData() {
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    // Group B: Cleanup old data (created more than month ago and marked as deleted)
    ...
    if (SQL_SUCCEEDED(SQLExecDirectW(hStmt, query, SQL_NTS))) {
        ...
        cout << "[CLEANUP] Deleted " << count << " old files from Recycle Bin." << endl;
